...
export default function Key(){
    ...
    useEffect(()=>{
        setKeyValid((!(typeof key == "undefined") && (key?.startsWith("sk_live_") || key?.startsWith("sk_test_"))))
    }, [key])
    return (<>
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className="bg-white dark:bg-black outline dark:outline-white w-[50vw] max-h-[85vh] z-[70] rounded-md fixed p-6 top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]">
                    ...
                    <Dialog.Description className="text-slate-400 mt-[10px] mb-[20px]">
                        ...
                        <input autoComplete='off' onChange={(e) => setKey(e.target.value)} type="password" id="password" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="sk_live_••••••••••••••••••" required />
